...
function threadHtml(t, opts) {
  ...
  var headline = t.rep
    ...
    : live
      ...
      : (t.repHarvested ? esc(t.repHarvested.summary)
...
function unitHtml(u, opts) {
  ...
  // The unit's harvested fallback: the newest run carrying one (runs are
  // newest-first). Only ever shown PLAIN on a finished unit — never as a
  // brief while live, where a mid-session reasoning line would masquerade
  // as a result.
  var harvested = null;
  u.runs.forEach(function (r) { if (!harvested && r.repHarvested) harvested = r.repHarvested; });
  var headline = u.rep
    ? esc(u.rep.rep.summary)
    : u.live
      ? '<span style="color:var(--amber)">Working on:&nbsp;</span>' +
        (newest.ask ? esc(newest.ask) : '<span style="color:var(--text3)">(prompt not shared)</span>')
      : (harvested ? esc(harvested.summary)
        : newest.ask ? esc(newest.ask)
        : '<span style="color:var(--text3)">session ended · no summary shared</span>');
  ...
  var head = '<div data-card-toggle="' + keyAttr + '" style="display:flex;gap:14px;align-items:flex-start;padding:2px 12px 2px 4px;margin:0 -12px 0 -4px;cursor:pointer;border-radius:14px;transition:background .2s" style-hover="background:var(--surface2)">' +
    '<div style="width:28px;height:28px;border-radius:10px;background:' + color + ';color:#fff;font-size:12px;font-weight:600;display:flex;align-items:center;justify-content:center;flex:none;margin-top:1px;box-shadow:var(--shadow-md)">' + initial + '</div>' +
    '<div style="flex:1;min-width:0">' + liveLabel +
      '<div class="sess-link" data-sess-open="' + navKey + '" style="font-size:15px;font-weight:600;letter-spacing:-0.01em;line-height:1.45;text-wrap:pretty">' + headline + '</div>' +
      '<div style="display:flex;align-items:center;gap:9px;margin-top:7px;font-size:12px;color:var(--text3);flex-wrap:wrap">' +
        '<span style="color:var(--text2);font-weight:500">' + esc(who) + '</span>' +
  ...
  var threads = u.runs.slice().reverse().map(function (r, i) {
    ...
    var rlabel = r.rep
      ...
      : rlive
        ? '<span style="color:var(--amber)">Working on:&nbsp;</span>' +
          (r.entries[0].ask ? esc(r.entries[0].ask) : '<span style="color:var(--text3)">(prompt not shared)</span>')
        : (r.repHarvested ? esc(r.repHarvested.summary)
          : r.entries[0].ask ? esc(r.entries[0].ask)
          : '<span style="color:var(--text3)">session ended · no summary shared</span>');
  ...
  var body = '<div style="display:' + (open ? 'block' : 'none') + '">' +
    '<div style="margin:2px 0 6px 13px;padding-left:12px">' + threads + '</div>' + cardCloseHtml(keyAttr) + '</div>';
...
function loadSession() {
  ...
  fetch('/api/feed?limit=100').then(function (r) { return r.json(); }).then(function (f) {
    ...
    // in place on the next tick.
    var fp = JSON.stringify({ k: key, t: t && t.entries, s: subs.map(function (x) { return x.entries; }), off: offline });
...
// One-line label for a subagent run: distilled summary, else harvested, else
// what it touched. Never a prompt list — the detail lives on that run's page.
function subagentLine(r) {
  if (r.rep) return esc(r.rep.summary);
  if (r.repHarvested) return esc(r.repHarvested.summary);
  var seen = {}, files = [];
...
// when the entry carries it, the clipped summary otherwise) with the live
// badge while in progress, author · tool · project · started meta, the
// numbered prompt timeline oldest→newest with a collapsed per-prompt subagent
// dropdown (siblings from the same work unit, attributed by timing), a files

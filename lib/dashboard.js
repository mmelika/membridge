...
function applyNav(t) {
  var map = { projects: t === 'home', feed: t === 'feed', team: t === 'team' };
...
// Placeholder Team screen. The full v3 Team surface is built in a later task;
// for now the nav routes here and lands on a lightweight placeholder that points
// at the existing team management in Settings.
function renderTeamScreen() {
  ...
  host.innerHTML =
    '<main style="max-width:660px;margin:0 auto;padding:48px 28px 110px;animation:mbFade .4s cubic-bezier(.16,1,.3,1)">' +
      ...
      '<div style="color:var(--text3);font-size:12.5px;margin-bottom:20px">The dedicated team surface is on its way. Meanwhile, ' +
        '<span data-team-to-settings style="color:var(--accent);cursor:pointer;font-weight:600">manage your team in Settings</span>.</div>' +
    '</main>';
}
document.getElementById('teamScreen').addEventListener('click', function (e) {
  if (e.target.closest('[data-team-to-settings]')) { location.hash = '#settings'; }
});
...
function loadSettings() {
  ...
  fetch('/api/team').then(function (r) { return r.json(); }).catch(function () { return null; }).then(function (t) {
    ...
    if (pendingInviteScroll) { pendingInviteScroll = false; stInviteOpen = true; }
    renderSettingsView();
    if (stInviteOpen) scrollToInvite();
  }).catch(function () { setPill(false); });
}
...
function renderSettingsView() {
  ...
  host.innerHTML =
    '<main style="max-width:660px;margin:0 auto;padding:48px 28px 110px;animation:mbFade .4s cubic-bezier(.16,1,.3,1)">' +
    '<div data-nav-catchup style="font-size:12.5px;color:var(--text3);cursor:pointer;margin-bottom:16px;display:inline-block;transition:color .2s" style-hover="color:var(--text2)">&larr; Catch-Up</div>' +
...
function settingsTeamSection() {
  ...
  if (!t || !t.authenticated) {
    return lbl + '<div style="' + STCARD + ';margin-bottom:34px;padding:15px 18px">' +
      '<div style="font-size:13px;color:var(--text2)">You&rsquo;re working locally. ' +
      '<span data-go-signin style="color:var(--accent);cursor:pointer;font-weight:600">Sign in</span> ' +
      'to create or join a team and share memory.</div></div>';
  }
  var team = pickTeam(t);
  if (!team) {
    return lbl + '<div style="' + STCARD + ';margin-bottom:34px;padding:15px 18px">' +
      '<div style="font-size:14px;font-weight:600;margin-bottom:4px">You&rsquo;re not in a team yet</div>' +
      '<div style="font-size:12.5px;color:var(--text3);margin-bottom:12px">Create a workspace or join one with an invite code. Until then your AI activity stays local to this Mac.</div>' +
      '<span data-st-create style="font-size:13px;color:var(--accent);cursor:pointer;font-weight:600">+ Create a team</span>' +
      '<span style="margin:0 12px;color:var(--text3)">&middot;</span>' +
      '<span data-st-join style="font-size:13px;color:var(--accent);cursor:pointer;font-weight:600">Join with a code</span></div>';
  }
  var teams = t.teams || [];
  ...
  var nameCell;
  if (teams.length > 1) {
    nameCell = '<select data-team-change="switch" aria-label="Switch team" style="font-size:14px;font-weight:600;border:1px solid var(--border);border-radius:8px;background:var(--card);color:var(--text);padding:4px 8px;font-family:inherit">' +
      teams.map(function (tm) {
        return '<option value="' + esc(tm.team_id) + '"' + (tm.team_id === team.team_id ? ' selected' : '') + '>' + esc(tm.team_name) + '</option>';
      }).join('') + '</select>';
  } else {
    nameCell = '<div style="font-weight:600;font-size:14px">' + esc(team.team_name) + '</div>';
  }
  var head =
    '<div style="display:flex;align-items:center;gap:12px;padding:15px 18px;border-bottom:1px solid var(--border)">' +
      '<div style="flex:1">' + nameCell +
        '<div style="font-size:12px;color:var(--text3);margin-top:2px">' + count + ' member' + (count === 1 ? '' : 's') + created + '</div></div>' +
      '<button data-st-rename style="height:34px;padding:0 13px;border-radius:10px;border:1px solid var(--border);background:var(--card);color:var(--text2);font-size:12px;font-weight:500;cursor:pointer;font-family:inherit;transition:all .2s" style-hover="border-color:var(--accent-brd);color:var(--text)">Rename</button>' +
    '</div>';
  var self = t.user;
  var rows = stMembers.map(function (m) {
    var isSelf = self && self.userId === m.user_id;
    var owner = m.role === 'owner';
    var badgeStyle = owner
      ? MONO + ';font-size:9.5px;letter-spacing:.08em;text-transform:uppercase;color:var(--accent);border:1px solid var(--accent-brd);background:var(--accent-soft);border-radius:99px;padding:3px 10px'
      : MONO + ';font-size:9.5px;letter-spacing:.08em;text-transform:uppercase;color:var(--text3);border:1px solid var(--border);border-radius:99px;padding:3px 10px';
    var sub = isSelf ? ('you' + (self.email ? ' &middot; ' + esc(self.email) : '')) : (m.email ? esc(m.email) : '');
    var canManage = team.role === 'owner' && !owner && !isSelf;
    var controls = '';
    if (canManage) {
      controls += '<select data-team-change="set-role" data-user-id="' + esc(m.user_id) + '" aria-label="Role" style="' + MONO + ';font-size:10px;border:1px solid var(--border);border-radius:8px;background:var(--card);color:var(--text2);padding:2px 6px">' +
        '<option value="member"' + (m.role === 'member' ? ' selected' : '') + '>member</option>' +
        '<option value="admin"' + (m.role === 'admin' ? ' selected' : '') + '>admin</option></select>';
      controls += '<span data-team-action="remove-member" data-user-id="' + esc(m.user_id) + '" style="font-size:12px;color:var(--text3);cursor:pointer" style-hover="color:#DC2626">Remove</span>';
    }
    return '<div style="display:flex;align-items:center;gap:13px;padding:13px 18px;border-bottom:1px solid var(--border)">' +
      '<div style="width:28px;height:28px;border-radius:10px;background:' + avColor(m.user_id) + ';color:#fff;font-size:11px;font-weight:600;display:flex;align-items:center;justify-content:center">' + esc(((m.display_name || '?').charAt(0) || '?').toUpperCase()) + '</div>' +
      '<div style="flex:1"><span style="font-size:13.5px;font-weight:600">' + esc(m.display_name) + '</span> <span style="font-size:12px;color:var(--text3)">' + sub + '</span></div>' +
      controls +
      '<span style="' + badgeStyle + '">' + esc(m.role) + '</span></div>';
  }).join('');
  var inviteRow = '';
  if (stInviteOpen && team.invite_code) {
    inviteRow = '<div id="settings-invite" style="display:flex;align-items:center;gap:9px;padding:12px 18px;border-bottom:1px solid var(--border);background:var(--surface2)">' +
      '<span style="' + MONO + ';font-size:12px;color:var(--text2);flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + esc(team.invite_code) + '</span>' +
      '<button data-team-action="copy-invite" data-code="' + esc(team.invite_code) + '" style="padding:6px 12px;border-radius:8px;border:none;background:var(--grad);color:#fff;font-size:12px;font-weight:600;cursor:pointer;font-family:inherit">Copy code</button></div>';
  }
  var foot =
    '<div style="display:flex;align-items:center;gap:12px;padding:13px 18px">' +
      '<span data-st-invite style="font-size:13px;color:var(--accent);cursor:pointer;font-weight:600">+ Invite with a code</span>' +
      '<div style="flex:1"></div>' +
      (team.role !== 'owner' ? '<span data-team-action="leave" style="font-size:12.5px;color:var(--text3);cursor:pointer;transition:color .2s" style-hover="color:#DC2626">Leave team</span>' : '') +
    '</div>';
  return lbl + '<div style="' + STCARD + ';margin-bottom:34px">' + head + rows + inviteRow + foot + '</div>';
}
...
if (settingsRoot) {
  settingsRoot.addEventListener('click', function (e) {
    if (e.target.closest('[data-nav-catchup]')) { location.hash = '#home'; return; }
    if (e.target.closest('[data-go-signin]')) { location.hash = '#signin'; return; }
    if (e.target.closest('[data-st-rename]')) {
      var team = pickTeam(stTeam);
      if (!team) return;
    ...
    if (e.target.closest('[data-st-invite]')) { stInviteOpen = !stInviteOpen; renderSettingsView(); return; }
    if (e.target.closest('[data-st-create]')) {
      var tn = window.prompt('New team name');
      if (tn && tn.trim()) {
        teamRequest('/api/team/create', { name: tn.trim() }).then(function (r) {
          if (r.team_id) rememberTeam(r.team_id); loadSettings();
        }).catch(function () { setPill(false); });
      }
      return;
    }
    if (e.target.closest('[data-st-join]')) {
...
function renderProject(feed, detail, value) {
  ...
  var head =
    '<div class="pj-back pj-close" title="Back to Catch-Up" style="font-size:12.5px;color:var(--text3);cursor:pointer;margin-bottom:16px;display:inline-block">&larr; Catch-Up</div>' +
  ...
  var h = head + pausedBanner + stats + notice +

...
document.getElementById('view-feed').addEventListener('click', function (e) {
  ...
  if (shareEl) {
    ...
    fetch('/api/share-session', {
      ...
      .then(function () { feedFp = ''; loadFeed(); }) // re-fetch the feed so the toggle + prompt visibility refresh
...
pjRoot.addEventListener('click', function (e) {
  ...
  if (shareEl) {
    ...
    fetch('/api/share-session', {
      ...
      .then(function () { pjFp = ''; loadProject(); }) // re-fetch the project stream so the toggle + prompt visibility refresh
      .catch(function () { setPill(false); shareEl.removeAttribute('data-busy'); });
    return;
  }

...
// Electron BrowserWindow. Dark-only, Uplinq-inspired design.
//
...
// Tool colors: Claude Code teal, Codex cyan, anything else violet.
var toolHex = function (src) {
  if (/claude/i.test(src || '')) return '#2de0a7';
  if (/codex/i.test(src || '')) return '#38b6ff';
  return '#b48cff';
};
...
function applyPjTab() {
  var names = ['activity', 'memory', 'plan'];

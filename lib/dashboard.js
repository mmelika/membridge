...
<header style="position:sticky;top:0;z-index:40;background:var(--bg);border-bottom:1px solid var(--border)">
  <div style="max-width:1080px;margin:0 auto;padding:0 28px;height:56px;display:flex;align-items:center;gap:10px">
    ...
    <button id="openSettings" title="Settings" aria-label="Settings" style="width:38px;height:38px;border-radius:10px;border:1px solid var(--border);background:var(--card);color:var(--text2);cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all .2s" style-hover="border-color:var(--accent-brd);color:var(--text)">
      <svg width="20" height="20" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4"><circle cx="8" cy="8" r="2.4"></circle><circle cx="8" cy="8" r="6" stroke-dasharray="1.8 2.4"></circle></svg>
    </button>
  </div>
</header>
...
function renderProjectsIndex() {
  ...
  // with the team → dropdown. Same pxFilter state and re-render as before.
...
function ensureFeedMembers() {
  ...
  fetch('/api/team').then(function (r) { return r.json(); }).then(function (t) {
    ...
    return Promise.all(teams.map(function (tm) {
      ...
      var seen = {}, out = [];
      lists.forEach(function (ms) { ms.forEach(function (m) { if (m.user_id && !seen[m.user_id]) { seen[m.user_id] = 1; out.push({ id: m.user_id, name: m.display_name }); } }); });
      feedMembers = out;
    });
...
// grow with the team / project count → labeled dropdowns. Values are exactly
// the old chips' values (author = user_id, project = name, source = string;
// '' = All = null), so the feedFilters contract and /api/feed queries are
// unchanged — only the control shape is different.
...
function feedFilterBarHtml(entries) {
  ...
  var projOpts = [all].concat(Object.keys(projects).map(function (k) { return { label: projects[k], val: k }; }));
  var toolOpts = [all].concat(Object.keys(tools).map(function (k) { return { label: tools[k], val: k }; }));
...
// One session widget. Collapsed = the AI brief (a .sess-link that routes to
// the session page) + meta row (author · tool pill · project · time ·
// "N prompts ▾") — no prompt peek. In-progress (no summary yet) = accent left
// border + pulsing amber "working now" + amber "Working on: <latest ask>"
// headline. The dropdown reuses the data-card-toggle contract (header + next
// sibling + catchupExpanded), so an open dropdown survives the 5s poll and the
// chevron must stay the LAST span in the header; the headline's data-sess-open
// is matched BEFORE the toggle in the click handler, so clicking the brief
// navigates without toggling and clicking the count toggles without navigating.
function threadHtml(t) {
  ...
  var headline = t.rep
    ? esc(t.rep.summary)
    : '<span style="color:var(--amber)">Working on:&nbsp;</span>' +
      (newest.ask ? esc(newest.ask) : '<span style="color:var(--text3)">(prompt not shared)</span>');
  var head = '<div data-card-toggle="' + keyAttr + '" style="display:flex;gap:14px;align-items:flex-start;padding:17px 12px 17px 4px;margin:0 -12px 0 -4px;cursor:pointer;border-radius:14px;transition:background .2s" style-hover="background:var(--surface2)">' +
    '<div style="width:28px;height:28px;border-radius:10px;background:' + color + ';color:#fff;font-size:12px;font-weight:600;display:flex;align-items:center;justify-content:center;flex:none;margin-top:1px;box-shadow:var(--shadow-md)">' + initial + '</div>' +
    '<div style="flex:1;min-width:0">' +
      '<div class="sess-link" data-sess-open="' + keyAttr + '" style="font-size:15px;font-weight:600;letter-spacing:-0.01em;line-height:1.45;text-wrap:pretty">' + headline + '</div>' +
      '<div style="display:flex;align-items:center;gap:9px;margin-top:7px;font-size:12px;color:var(--text3);flex-wrap:wrap">' +
        '<span style="color:var(--text2);font-weight:500">' + esc(who) + '</span>' +
        ...
        (wip ? '<span style="display:flex;align-items:center;gap:5px;color:var(--amber);font-weight:600;font-size:11.5px"><span style="width:6px;height:6px;border-radius:50%;background:var(--amber);animation:mbPulse 2s ease infinite"></span>working now</span>' : '') +
      ...
      '</div>' +
    '</div></div>';
  // Capped dropdown: the 3 MOST RECENT prompts (shown oldest→newest of those
  // three), each clamped to 2 lines — "show more" is unhidden by measurement
  // (markClamped) only when the text truly overflows, and expands just that
  // prompt in place. Worst case the card is headline + 3 clamped rows + the
  // "See all" link; it never grows unbounded. A null ask stays
  // "(prompt not shared)" — redaction already happened server-side.
  ...
  var kidsBlock = '<div style="display:' + (open ? 'block' : 'none') + '">' +
    '<div style="margin:2px 0 18px 13px;padding:2px 0 2px 25px;border-left:2px solid var(--border)">' + kids + seeAll + '</div></div>';
  return '<article style="border-bottom:1px solid var(--border)' + (wip ? ';border-left:3px solid var(--accent);padding-left:10px' : '') + '">' + head + kidsBlock + '</article>';
}
...
// (lines 350); a thread lands under the day of its newest entry.
function feedDayGroupHtml(entries) {
  var threads = buildThreads(entries);
  var h = '', lastDay = '';
  for (var i = 0; i < threads.length; i++) {
    var day = homeDayLabel(threads[i].ts);
    if (day !== lastDay) {
...
function loadSession() {
  ...
  fetch('/api/feed?limit=100').then(function (r) { return r.json(); }).then(function (f) {
    ...
    // Serializing the whole thread covers prompt count AND summary arrival, so
    // a live session re-renders the moment anything new lands.
    var fp = JSON.stringify({ k: key, t: t && t.entries, off: offline });
    if (fp === sessFp && host.firstChild) { refreshAgo('view-session'); return; }
    sessFp = fp;
    host.innerHTML = sessionPageHtml(t);
  }).catch(function () { setPill(false); });
}
...
// Pure string builder (also exercised by the fixture checks): brief as the
// page header with the live badge while in progress, author · tool · project ·
// started meta, the FULL untruncated prompt timeline oldest→newest, a files
// section only when the entries carry files, and a live "working…" tail.
function sessionPageHtml(t) {
  ...
  var head =
    '<div style="display:flex;align-items:center;gap:10px;margin-bottom:12px">' +
      '<span style="' + MONO + ';font-size:10px;letter-spacing:.15em;text-transform:uppercase;color:var(--text3)">Session</span>' + live +
    '</div>' +
    ...
    '<div style="display:flex;align-items:center;gap:10px;font-size:12.5px;color:var(--text3);flex-wrap:wrap;padding-bottom:22px;border-bottom:1px solid var(--border)">' +
  ...
  // shared)" — redaction already happened server-side.
  var rows = t.entries.slice().reverse().map(function (e, i) {
    return '<div style="display:flex;gap:12px;padding:10px 0;align-items:baseline">' +
  ...
  var filesHtml = files.length
    ...
    : '';
  return openMain + SESS_BACK + head + timeline + filesHtml + '</main>';
}
// Back to Activity; delegated so it survives the 5s re-render.
document.getElementById('view-session').addEventListener('click', function (e) {
  if (e.target.closest('[data-sess-back]')) location.hash = '#everything';
});

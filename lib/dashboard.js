...
// same normalization as unitKeyOf plus the local day. units arrive already
// newest-first (buildUnits' contract), so a single grouping pass (first unit
// seen per key anchors the card's identity fields) both accumulates the
// rollup and leaves cards newest-first with no extra sort. Nothing on a unit
// is mutated; cards only hold references into the input array. ----
function buildDayCards(units) {
  ...
  cards.forEach(function (c) {
    ...
    c.units.forEach(function (u) {
      if (!u.rep || !u.rep.rep) return;
      if (!pick || u.promptCount > pick.promptCount ||
        (u.promptCount === pick.promptCount && String(u.ts) > String(pick.ts))) pick = u;
    });
    c.headline = pick
      ? runHeadline(pick.rep && pick.rep.rep, pick.runs[0].entries[0], pick.live)
      : (c.live
  ...
  cards.forEach(function (c) {
    ...
    c.units.forEach(function (u) {
      var glyph = u.live ? '◐' : (u.rep && u.rep.rep ? '✓' : '○');
      rows.push({ glyph: glyph, text: runHeadline(u.rep && u.rep.rep, u.runs[0].entries[0], u.live), live: !!u.live });
...
function dayDetailHtml(c, opts) {
  ...
  var cards = ordered.map(function (u) {
    var repEntry = u.rep && u.rep.rep;
    var title = runHeadline(repEntry, u.runs[0].entries[0], u.live);
    var liveLabel = u.live
      ? '<div style="display:flex;align-items:center;gap:6px;margin-bottom:5px;' + MONO + ';font-size:9.5px;letter-spacing:.12em;text-transform:uppercase;color:var(--amber);font-weight:600"><span style="width:6px;height:6px;border-radius:50%;background:var(--amber);animation:mbPulse 2s ease infinite;flex:none"></span>Working now</div>'
      : '';
    var summary = (repEntry && (repEntry.summaryFull || repEntry.summary))
      ? '<div style="font-size:13px;color:var(--text2);line-height:1.55;margin-top:6px">' + esc(repEntry.summaryFull || repEntry.summary) + '</div>'
      : '';
    ...
    var rows = entries.map(function (e2) {
      return '<div style="display:flex;gap:10px;align-items:baseline;padding:6px 0">' +
        '<div style="flex:1;min-width:0;font-size:13px;line-height:1.55;' + (e2.ask ? 'color:var(--text2);font-style:italic' : 'color:var(--text3)') + '">' +
          (e2.ask ? '&ldquo;' + esc(e2.ask) + '&rdquo;' : '(prompt not shared)') + '</div>' +
    ...
    var foot = '<div style="display:flex;align-items:center;justify-content:space-between;gap:10px;margin-top:9px">' +
      ...
      '<span style="font-size:11.5px;color:var(--text3);flex:none" data-ago="' + esc(u.ts) + '">' + esc(ago(u.ts)) + '</span>' +
    '</div>';
  ...
  if (shareEl) {
    ...
    fetch('/api/share-session', {
      ...
      .then(function (b) {
        ...
        // so on failure the reloaded badge reverts to its true state; flag the
        // sync pill unhealthy too rather than letting the toggle silently snap back.
        if (b && b.ok === false) setPill(false);
...
function loadDay() {
  ...
  fetch('/api/feed?limit=100').then(function (r) { return r.json(); }).then(function (f) {
    ...
    var fp = JSON.stringify({ k: key, off: offline, live: !!(card && card.live), u: card && card.units.map(function (u) {
      return [u.key, u.live, u.promptCount, u.rep && u.rep.rep && (u.rep.rep.summaryFull || u.rep.rep.summary)];
    }) });
...
pjRoot.addEventListener('click', function (e) {
  ...
  if (shareEl) {
    ...
    fetch('/api/share-session', {
      ...
      .then(function (b) {
        if (b && b.ok === false) setPill(false); // backend rejected the change; badge reverts on reload

...
function renderTeam(d) {
  ...
  var teamRows = teams.map(function (t) {
    return '<div class="team-row"><div class="avatar">' + esc((t.team_name || 'T').charAt(0).toUpperCase()) + '</div>' +
      ...
      (t.role === 'owner' ? '<div class="invite">Invite · ' + esc(t.invite_code) + '</div>' : '') + '</div>' +
      (t.role === 'owner' ? '<button class="btn" data-team-action="copy-invite" data-code="' + esc(t.invite_code) + '">Copy invite</button>' : '') + '</div>';
  }).join('');
...
function handleTeamClick(e) {
  ...
  request.then(function (result) {
    ...
    } else if (action === 'link') setTeamNotice('Project linked. Its redacted memory is now shared with this team.', 'success');
...
function renderProject(d) {
  ...
  h += '<div class="ptabs">' +
    ...
    '<button class="ptab" data-ptab="team">Team' + (d.teamEntries.length ? ' · ' + esc(d.teamEntries.length) : '') + '</button></div>';
...
function applyPjTab() {
  var names = ['activity', 'memory', 'team'];

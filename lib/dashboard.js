...
<header>
  ...
  <button class="btn" id="openScan" title="Detected tools">Scan</button>
...
document.getElementById('openScan').onclick = openScan;
...
function renderTeam(d) {
  ...
  var teamRows = teams.map(function (t) {
    ...
    var activeInvite = teamInvites[t.team_id];
    return '<div class="team-row"><div class="avatar">' + esc((t.team_name || 'T').charAt(0).toUpperCase()) + '</div>' +
      ...
      (activeInvite ? '<div class="invite">Invite link · ' + esc(activeInvite.token) + '</div>' : (t.role === 'owner' ? '<div class="invite">Legacy code · ' + esc(t.invite_code) + '</div>' : '')) + '</div>' +
      (manager ? '<button class="btn primary" data-team-action="invite-options" data-team-id="' + esc(t.team_id) + '">' + (inviteOptionsTeamId === t.team_id ? 'Hide options' : 'Create invite') + '</button>' : '') +
      (manager && activeInvite ? '<button class="btn danger" data-team-action="revoke-invite" data-team-id="' + esc(t.team_id) + '" data-token="' + esc(activeInvite.token) + '">Revoke</button>' : '') +
      (t.role === 'owner' ? '<button class="btn" data-team-action="copy-invite" data-code="' + esc(t.invite_code) + '">Copy code</button>' : '') +
      (manager && inviteOptionsTeamId === t.team_id
        ? '<form class="team-form invite-options" data-team-form="invite-options" data-team-id="' + esc(t.team_id) + '">' +
          '<label>Expires in<input name="expiresDays" inputmode="numeric" pattern="[0-9]*" placeholder="7"><span class="m-help">days, blank for none</span></label>' +
          '<label>Max uses<input name="maxUses" inputmode="numeric" pattern="[0-9]*" placeholder="unlimited"><span class="m-help">blank for unlimited</span></label>' +
...
function handleTeamClick(e) {
  ...
  if (action === 'invite-options') {
    inviteOptionsTeamId = inviteOptionsTeamId === btn.dataset.teamId ? '' : btn.dataset.teamId;
    loadTeam();
    return;
  }
  if (action === 'revoke-invite') {
    if (!confirm("Revoke this invite link? Anyone who hasn't used it yet won't be able to join.")) return;
    btn.disabled = true;
    teamRequest('/api/team/revoke-invite', { token: btn.dataset.token }).then(function () {
      delete teamInvites[btn.dataset.teamId];
      setTeamNotice('Invite link revoked.', 'success');
      loadTeam();
    }).catch(function (err) {
...
function handleTeamSubmit(e) {
  ...
  } else if (kind === 'invite-options') {
    path = '/api/team/invite';
    body = {
      teamId: form.dataset.teamId,
      expiresDays: data.get('expiresDays'),
  ...
  teamRequest(path, body).then(function (result) {
    if (kind === 'invite-options') {
      var text = result.url || ('membridge join ' + result.token);
      return copyText(text).then(function () {
        teamInvites[form.dataset.teamId] = result;
        inviteOptionsTeamId = '';
        setTeamNotice(result.url ? 'Invite link created and copied.' : 'Invite command created and copied.', 'success');
        loadTeam();
      });
    }

...
function feedEntryHtml(e, opts) {
  ...
  if (e.summary) {
    body = '<div class="fsummary' + (e.distilled ? ' distilled' : '') + '">' + esc(e.summary) + '</div>'
      + '<div class="fask">Asked: ' + esc(e.ask || '(prompt not shared)') + '</div>';
  } else {
    body = '<div class="fworking">Working on: ' + esc(e.ask || '(prompt not shared)') + ' <span class="fhint">in progress</span></div>';
  }

...
var setPill = function (ok, teamLastSync) {
  ...
  pillEl.textContent = lastTeamSync ? 'Synced · ' + ago(lastTeamSync) : 'Synced';
...
// "Sign in" only shows when signed-out; the auth view already fills the screen
// in that state, so this just makes sure we are on it. (Full flow: a later task.)
var signinBtn = document.getElementById('openSignin');
if (signinBtn) signinBtn.onclick = function () { if (location.hash) location.hash = ''; };
...
// First-run Welcome: composes /api/status + /api/scan. Reuses openAdd/openScan.
function renderWelcome(host) {
  Promise.all([
    ...
    host.innerHTML =
      '<div class="card" style="padding:28px 30px"><span class="section-label">Welcome to MemBridge</span>' +
      ...
      '<div class="cu-steps">' +
        '<div class="cu-step"><span class="cu-ok">&#10003;</span><div class="grow"><strong>Daemon running</strong><span class="cu-dim"> — watching for sessions</span></div></div>' +
        '<div class="cu-step"><span class="cu-ok">&#10003;</span><div class="grow"><strong>' + (n ? n + ' tool' + (n === 1 ? '' : 's') + ' detected' : 'No tools detected yet') + '</strong>' +
...
// Empty-state builders (mockup lines 149–178). head above already renders the
// section-label pill + split-accent title; these render the centered visual.
function allCaughtUpHtml(cu) {
  var when = cu.lastViewedTs ? ago(cu.lastViewedTs) : 'you last looked';
  return '<div class="cu-empty">' +
    '<div class="cu-tile">&#10003;</div>' +
    '<div class="cu-empty-title">You&#8217;re all caught up</div>' +
    '<p class="cu-empty-sub">Nothing new since ' + esc(when) + '. New work will collect here as it happens.</p>' +
    (cu.prevViewedTs ? '<button class="linklike cu-undo" data-catchup="undo">Undo</button>' : '') +
    '</div>';
}
function noTeamCatchupHtml() {
  return '<div class="card cu-noteam">' +
    '<div class="cu-avs2"><span class="cu-av-solid"></span><span class="cu-av-dash"></span></div>' +
    '<div class="cu-empty-title">You&#8217;re not on a team yet</div>' +
    '<p class="cu-empty-sub">The catch-up comes alive when a teammate joins — their sessions get distilled into your briefing, and yours into theirs. Until then, MemBridge keeps syncing memory into your own tools.</p>' +
    '<button class="btn primary" data-catchup="go-team">Create an invite link</button>' +
    '<div class="cu-join">or <button class="linklike" data-catchup="go-team">join an existing team</button> with a code</div>' +
    '</div>';
}

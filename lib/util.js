...
// catch Claude's temp roots. os.tmpdir() itself is deliberately NOT used here:
// the test suite puts its own fixtures under os.tmpdir(), so matching it would
// classify every test project as temp.
function isTempPath(p) {
  if (!p) return false;
  const n = normPath(p);
  if (n.split(path.sep).includes('scratchpad')) return true;
  return n.startsWith('/private/tmp/') || n.startsWith('/tmp/claude-');
}
...
module.exports = {
  ...
  DEFAULT_CONFIG, EXTRA_TARGETS, STATE_VERSION, DEFAULT_CATCHUP, ensureConfig, loadUserConfig, saveUserConfig, getConfig,

...
const DEFAULT_CONFIG = {
  $docs: 'MemBridge config. targets: context files injected per project. exclude: project paths/globs to skip. redact: regexes scrubbed before injection. adapters.custom: point MemBridge at any JSONL session store (see README). graph: neural-map tuning. team: Supabase backend for team sync (set via `membridge team setup`). advisor: BYOK roadmaps key + planner model.',
  intervalSec: 60,
  dashboardPort: 7437,

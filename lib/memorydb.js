...
function buildEntries(projectPath, proj, config) {
  ...
  // entry of the same session. Overwriting keeps only the latest of each —
  // earlier self-reports and todo lists are stale by definition.
  ...
  for (const ev of proj.events) {
    if (ev.kind === 'prompt') {
      ...
      if (entry) entry.summary = digest.clip(digest.redactText(digest.plainText(ev.text), regexes), 300);

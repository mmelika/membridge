...
// A short human-comparable fingerprint of a box public key, for Signal-style
// out-of-band verification: generichash the raw key, render 16 bytes as eight
// 4-hex groups. The format is a contract — two humans read these aloud to
// each other — so it must stay stable across releases.
function fingerprint(publicKey) {
  need();
  const h = sodium.crypto_generichash(32, un64(publicKey));
  return sodium.to_hex(h.slice(0, 16)).match(/.{4}/g).join(' ');
}

module.exports = {
  ready, available, genKeypair, genTeamKey,
  sealTeamKey, unsealTeamKey, encrypt, decrypt, fingerprint,
};

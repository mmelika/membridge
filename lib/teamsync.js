...
async function pushProject(config, creds, projectPath, proj, link, crypto) {
  ...
  // machine only when the user opts in (config.team.sharePrompts, same
  // convention as team.autoLink). Summary and files upload either way.
  ...
  for (let i = 0; i < entries.length; i += PUSH_BATCH) {
    const plainRows = entries.slice(i, i + PUSH_BATCH).map(e => ({
      ...
      // decisions/gotchas are OUTCOME, not verbatim prompt — ship regardless
      // of sharePrompts, same as summary.
      decisions: e.decisions ? scrub(e.decisions, 240) : null,
      gotchas: e.gotchas ? scrub(e.gotchas, 240) : null,
      files: e.files,
      // The change model rides in its own column, mirroring the local data
      // model exactly (files stays string[] everywhere). Overloading `files`
      // with objects would render as "[object Object]" in every consumer that
      // treats a team row's files as filenames (digest, advisor, mcp).
    ...
    // A backend whose schema predates the summary/goal/decisions/gotchas/
    // changes columns (PostgREST PGRST204) must not kill team sync: drop
    // each missing field and retry until the insert lands (or a non-column
    // error surfaces). Stateless on purpose — these fields resume by
    // themselves once the backend gains the columns.
    let attempt = rows;
    for (;;) {
      try {
        ...
        // The three 009 columns join the drop list: a flag-on push against a
        // backend missing that migration degrades to plaintext-only rows
        // instead of failing the project's push.
...
module.exports = {
  ...
  ensureIdentity, resolveTeamKey, encryptRow,

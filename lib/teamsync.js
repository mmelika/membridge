...
async function pushProject(config, creds, projectPath, proj, link) {
  ...
  for (let i = 0; i < entries.length; i += PUSH_BATCH) {
    ...
    // A backend whose schema predates the summary/goal/changes columns
    // (PostgREST PGRST204) must not kill team sync: drop each missing field
    // and retry until the insert lands (or a non-column error surfaces).
    // Stateless on purpose — these fields resume by themselves once the
    // backend gains the columns.
    let attempt = rows;
    for (;;) {
      try {
        ...
        const m = /'(summary|goal|changes)' column/i.exec(err.message);
        if (!m) throw err;
        const drop = m[1];
...
async function pullProject(config, creds, proj, link) {
  ...
  const q = `memory_entries?project_id=eq.${link.projectId}` +
    ...
    '&select=author_name,ts,source,session,ask,goal,summary,files,changes,created_at';

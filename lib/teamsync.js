...
// and the redirect hands the session back in the URL fragment; the callback
// page posts those tokens here. The redirect target must be on the Supabase
...
async function loginWithTokens(config, accessToken, refreshToken, expiresIn) {
  ...
  // GitHub accounts carry full_name/user_name metadata instead of display_name.
  const m = user.user_metadata || {};
  const displayName =
    m.display_name || m.full_name || m.user_name || String(user.email || '').split('@')[0];
...
module.exports = {
  ...
  buildCryptoContext, decryptTeamRows, fingerprintReport, trustMember, rekeyTeam, reconcileTeamKeys,

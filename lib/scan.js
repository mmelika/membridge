  ...
  if (!opts.dryRun) {
    ...
    for (const key of captureKeys) {
      ...
      try {
        ...
        // Authorship gate: only commits this machine's identity committed may
        // be attributed to a local session. A pulled teammate commit (foreign
        // committer email) — or any commit when user.email is unset (fail
        // closed) — is recorded unattributed-locally, never falsely credited.
        const localEmail = commits.gitUserEmail(key);
        for (const sha of shas) {
          if (!recorded.has(sha)) {
            const c = commits.readCommit(key, sha);
            const att = commits.isLocalCommitter(c.email, localEmail)
              ? commits.attributeCommit(c.files, c.ts, proj.events || [], { projectPath: key })

...
// Git worktrees are the SAME project as their main repo. A worktree scanned
// before this rule minted its own state.projects entry; fold each such entry's
// events back into the main repo's project (deduped via mergeEvents) and drop
// the fragment, so the feed, context block, team push and ops-noise filter all
// see one project. Idempotent — once folded the fragment key is gone. Detection
// is authoritative from the worktree's `.git` pointer while it exists on disk,
...
function foldWorktreeProjects(state, config) {
  ...
  for (const K of Object.keys(projects)) {
    let mKey;
    try { mKey = mainKeyOf(K); } catch { mKey = null; }
    if (!mKey) continue;
...
module.exports = { readNewLines, getAdapters, scanAll, scanSummaries, syncOnce, findProjectKey, trackedRoots, filterTrackedSessions, filterScratchpadResidue, foldWorktreeProjects, cleanupCodexMislabels, reopenMislabeledForeignCodex, SETTLE_GRACE_MS };

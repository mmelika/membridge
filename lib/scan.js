...
function cleanupCodexMislabels(state, deps) {
  const readFirstEntry = (deps && deps.readFirstEntry) || (file => {
    // First COMPLETE, non-blank line. 1MB bound: genuine session_meta lines
    // carry the full base instructions (~20KB observed) — headroom, not a
    // hard shape limit. Anything unreadable/unparseable returns null =
    // "cannot judge", which leaves the file and its events alone.
    try {
      const fd = fs.openSync(file, 'r');
      try {
...
function syncOnce(opts = {}) {
  ...
  // Drop throwaway edits (agent scratchpad / temp roots) at capture, before
  // homing, so they never mint or pollute a project. rehomeEvents also guards
  // defensively, but dropping here keeps them out of the pipeline entirely.
  const scanned = scanAll(state, config)
    .filter(ev => !(ev.kind === 'edit' && ev.file && isTempPath(ev.file)));
...
module.exports = { readNewLines, getAdapters, scanAll, scanSummaries, syncOnce, findProjectKey, trackedRoots, filterTrackedSessions, cleanupCodexMislabels, SETTLE_GRACE_MS };

  ...
  try {
    ...
    for (const pk of Object.keys(state.projects || {})) {
      for (const e of state.projects[pk].events || []) {
        ...
        if (!root) continue;
        const rk = util.normPath(root);
        const prev = editRoots.get(rk) || { count: 0, root };
        editRoots.set(rk, { count: prev.count + 1, root });
      }
    }
    let key = null, bestCount = 0;
    for (const v of editRoots.values()) if (v.count > bestCount) { key = v.root; bestCount = v.count; }
...
module.exports = {
  ...
  setupHooks, removeHooks, isHookInstalled, hookCommand,
};
